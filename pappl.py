...
class Example(QtGui.QWidget):
	...
	color=0
	load=0
	noGraph = "Aucun graphe selectionne"
	grapheLoc=[]
	...
	def initUI(self):
		...
		self.load = QtGui.QPushButton('Charger un graphe', self)
		self.load.resize(self.load.sizeHint())
		self.load.clicked.connect(self.loading)
		
		self.launch = QtGui.QPushButton('Lancer cytoscape', self)
		self.launch.resize(self.launch.sizeHint())
		self.launch.clicked.connect(self.lancement)
		
		self.display =QtGui.QPushButton('Afficher le graphe',self)
		self.display.resize(self.launch.sizeHint())
		...
		#name=QtGui.QLineEdit('Emplacement: ')
		
		self.reduc = QtGui.QPushButton('Reduire le graphe', self)
		self.reduc.resize(self.reduc.sizeHint())
		self.reduc.clicked.connect(self.reduction)
		self.reduc.setEnabled(False)
		
		self.graph=QtGui.QComboBox(self)
		self.graph.addItem(self.noGraph)
		self.graph.setEditable(True)
		self.graph.lineEdit().setAlignment(QtCore.Qt.AlignCenter)
		
		self.color=QtGui.QPushButton('Rechercher les colorations')
		self.color.resize(self.color.sizeHint())
		self.color.clicked.connect(self.colorations)
		self.color.setEnabled(False)
		
		
		self.quit= QtGui.QPushButton('Quitter')
		...
		grid.addWidget(self.launch,1,0)
		grid.addWidget(self.load,2,0)  
		grid.addWidget(self.graph,3,0)
		grid.addWidget(self.reduc,4,0)
		grid.addWidget(self.display,5,0)
		grid.addWidget(self.color,6,0)
		grid.addWidget(self.quit,7,0)
		
		
		
		
		self.setGeometry(300, 300, 350, 300)
		self.setWindowTitle('PAPPL')   
		self.setLayout(grid)
		self.show()
	...
	def alerte(a):
		msglabel = QtGui.QLabel("Attention Cytoscape.exe n'est pas lance! \nVeuillez lancer l'application avant d'afficher un graphe.")
	...
	def loading(self):
		nom=QtGui.QFileDialog.getOpenFileName(self, 'Open file', 'c:\\')
	
		
		if (len(nom)>0):
			if (self.graph.currentText()==self.noGraph):
				self.graph.removeItem(self.graph.currentIndex())
			self.fname = nom
			self.grapheLoc.append(nom)
			self.graph.addItem(os.path.basename(str(nom)))
			self.display.setEnabled(True)
			self.reduc.setEnabled(True)
		
		
	def reduction(self):
		reduced=os.path.splitext(self.fname)[0]+"-reduced.sif"
		self.compaction(self.fname,os.path.splitext(self.fname)[0]+"-reduced.sif", os.path.splitext(self.fname)[0]+"-hash.txt", os.path.splitext(self.fname)[0]+"-logic.txt")
	...
	def affichage(self):
		if not self.isRunning():
			...
			net1 = cy.network.create_from(self.grapheLoc[self.graph.currentIndex()])
